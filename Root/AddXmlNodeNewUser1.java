...
//import MD5.MD5;
//import AES.AES;
public class AddXmlNodeNewUser1 extends HttpServlet {
    Integer i=0;
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException,ServletException {
      try{
        ...
        for (Server i : svr) {
            ...
            name.appendChild(document.createTextNode(req.getParameter("username")));
            users.appendChild(name);

            Element pwd = document.createElement("password");
            pwd.appendChild(document.createTextNode(req.getParameter("password")));
            users.appendChild(pwd);

            Element fn = document.createElement("firstname");
            fn.appendChild(document.createTextNode(req.getParameter("firstname")));
            users.appendChild(fn);
            
            Element ln = document.createElement("lastname");
            ln.appendChild(document.createTextNode(req.getParameter("lastname")));
            users.appendChild(ln);
            
            Element add = document.createElement("address");
            add.appendChild(document.createTextNode(req.getParameter("address")));
            users.appendChild(add);
            
            Element cty = document.createElement("city");
            cty.appendChild(document.createTextNode(req.getParameter("city")));
            users.appendChild(cty);
            
            Element state = document.createElement("state");
            state.appendChild(document.createTextNode(req.getParameter("state")));
            users.appendChild(state);
            
            Element zc = document.createElement("zipcode");
            zc.appendChild(document.createTextNode(req.getParameter("zipcode")));
            users.appendChild(zc);
            
            Element ph = document.createElement("telephone");
            ph.appendChild(document.createTextNode(req.getParameter("telephone")));
            users.appendChild(ph);

			Element sq = document.createElement("secquestion");
            sq.appendChild(document.createTextNode(req.getParameter("secquestion")));
            users.appendChild(sq);

			Element sa = document.createElement("secanswer");
            sa.appendChild(document.createTextNode(req.getParameter("secanswer")));
